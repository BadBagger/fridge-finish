...
object DateTextParser {
    ...
    private fun parseIsoDates(text: String, today: LocalDate): List<LocalDate> = buildList {
        Regex("""(?<!\d)(20\d{2})[-/.:\s](\d{1,2})[-/.:\s](\d{1,2})(?!\d)""")
            ...
            .forEach { match ->
                ...
                )?.takeIf { plausible(it, today) }?.let { add(it) }
            }
    }

    private fun parseNumericDates(text: String, today: LocalDate): List<LocalDate> {
        val fullDates = buildList {
            Regex("""(?<!\d)(\d{1,2})[-/.:\s](\d{1,2})[-/.:\s](\d{2,4})(?!\d)""")
                ...
                .forEach { match ->
                    ...
                    makeDate(year, month, day)?.takeIf { plausible(it, today) }?.let { add(it) }
                }
        }
        ...
        return buildList {
            Regex("""(?i)(?:exp|expires|best by|use by|sell by)\D{0,12}(\d{1,2})[-/.:\s](\d{2,4})(?![-/.:\s]*\d)""")
                ...
                .forEach { match ->
                    ...
                    makeDate(year, month, 1)?.takeIf { plausible(it, today) }?.let { add(it) }
                }
        }
    }

    private fun parseCompactDates(text: String, today: LocalDate): List<LocalDate> = buildList {
    ...
    private fun parseMonthNameDates(text: String, today: LocalDate): List<LocalDate> = buildList {
